# I'm witchcat

#你好我是女巫的折耳猫

#这是我的技术栈
#编程语言：Java, Python
#框架：Spring Boot, fastapi,
#数据库：MySQL, MongoDB,pgqsl
#工具：Docker, Git
#前端：vue,react

#我现在的工作与学习 ✨
#🔭 目前正在做：深入学习 微服务架构 和 分布式系统，尤其是 fastapi 。
#🌱 近期目标：完善自己在 Java 和 Python 技术栈上的技能基础上并在研究金融工程。
#💬 可以和我讨论：Spring Boot, fastapi, 任何你感兴趣的技术话题！
#📫 联系方式：你可以通过 我的邮箱 与我联系，或者在 GitHub 上给我留言。
#⚡ 趣事：我对编程的热情与咖啡同在，周末喜欢在 GitHub 上探索新项目，也偶尔做一些开源贡献！

#如果你也对技术、编程或者开源有热情，不妨一起交流、合作！一起让技术变得更有趣！


#我的 GitHub 统计 
#GitHub Stats Top Langs

---

![](https://raw.githubusercontent.com/witchscottishfoldcat/witchcat/master/generated/overview.svg)
![](https://raw.githubusercontent.com/witchscottishfoldcat/witchcat/master/generated/languages.svg)


---
